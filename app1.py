...
#lOADING DATASET
df=pd.read_csv('CIC-DDoS-2019-DNS.csv')
df=df.iloc[:1]
...
encrypted_df = pd.DataFrame(np.array(empty).reshape(1,5),columns = column_names)

print("Encryption Completed and written as encryption.csv file")
encrypted_df.to_csv(r'encrypted.csv',index = False)
st.success("Encryption  Successfully !!!")   
print("decryption Completed and written as Decryption.csv file")

decrypted_df = pd.DataFrame(np.array(decrypt).reshape(1,5),columns =df.columns)
